...
Write only the dated `*-qa.md` in the exact registered `docs-ai/<work-key>-<slug>/` folder with target identity, commands/results, observed behavior, pass/fail counts, acceptance mapping, cleanup, blockers, and residual risk. Recognize an explicitly supplied historical layout as read fallback without rewriting it.
