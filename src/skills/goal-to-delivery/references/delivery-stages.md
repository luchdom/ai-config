...
Apply [design-gates.md](./design-gates.md) to all frontend/UI work. A pre-implementation design spec is required whenever a UI decision remains. A narrowly mechanical change may record that no new spec is required only when a binding source dictates every UI decision. Every changed rendered UI or interaction still requires a current post-implementation product-designer design conformance review before downstream gates can pass. Changes proven not to affect rendered UI or interaction—including documentation-only, backend-only, or configuration-only work—do not require these UI gates; classify by rendered effect, not file type.
