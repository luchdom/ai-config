...
| Memory | Preserves the state and knowledge needed to resume safely across sessions. | Linear stores durable work and decisions; `workflow.json` and `docs-ai/` retain delivery identity and evidence; the supervisor state home retains leases, reservations, journals, recovery state, and persistent worktrees; immutable repository records and commit markers retain curated knowledge while a derived index supports bounded retrieval. | Curated promotion, deterministic retrieval, opt-in untrusted context, lifecycle, and marker-based recovery are implemented and fixture-backed; seed curation and live autonomous loading remain disabled pending attended rollout. |
...
The [repository-memory module](src/skills/linear-delivery-loop/references/repository-memory.md) adds reviewed immutable records, marker-last atomic promotion, marker-derived rebuild after state-home loss, deterministic provenance-bearing query budgets, and opt-in stage context labeled as untrusted data. Repository-specific curation starts at [`docs/repository-memory/`](docs/repository-memory/README.md). It is not loaded implicitly by autonomous delivery and cannot grant workflow or mutation authority.
