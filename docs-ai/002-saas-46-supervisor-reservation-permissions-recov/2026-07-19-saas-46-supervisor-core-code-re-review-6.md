# SAAS-46 supervisor core — independent code rereview 6

## Target identity

- Review scope: exact current working tree; only the pre-Phase-A context validation delta and its tests were inspected. No implementation, tests, workflow registry, Linear state, Git/provider state, or ignored output was mutated by this reviewer.
...
- Target digest: `sha256:b1c78c56a9882cf805767afebb090ab7f379a8c22a82c1b1dcf90345468ae604`.

## Verdict

**FAIL — 1 P2 finding.** The rewritten-dirty baseline from rereview 5 now fails closed, but the underlying mutable-baseline issue remains for a clean rewritten destination HEAD.

## Finding

### P2-1 — A rewritten clean destination HEAD is still accepted as the original pre-Phase-A baseline

The repair correctly validates `priorEvidenceIds` as a sorted unique string list and requires a full destination observation with the exact repository/fingerprint fields and an empty-status digest (`src/skills/linear-delivery-loop/scripts/assembled_handoff.py:204-239`). This rejects the prior dirty rewrite. The observation is still read only from the mutable `.assembly.json` context, however; no digest or equivalent immutable binding for that context exists in the operation journal or another separately validated transition record. `_no_base_mutation_is_exact` therefore still proves only that the current destination equals the currently stored baseline, not that the baseline is the one captured before Phase A (`src/skills/linear-delivery-loop/scripts/assembled_handoff.py:444-460`).

A disposable public-command probe stopped after context persistence and before Phase A, changed `README.md` in the destination, committed the change so the destination became clean at a new HEAD, rewrote `context["destinationObservation"]` with that clean current observation, and submitted Recover through `cli.run_request`. Recover returned `restored/not-started`, left `recovery.status=clean`, and marked the original Handoff journal `failed`. The companion rewritten-dirty probe correctly raised `AssembledHandoffError`, left the original journal `pending`, and set recovery ambiguous.

This remains P2 because Phase A did not begin and no new authority is distributed, but tampered recovery evidence can still replace the historical baseline and silently terminalize the audit journal. Bind the exact context bytes/digest into a separately validated durable operation transition before Phase A, and require recovery to verify that binding before comparing the persisted baseline to current state. A clean-but-new-HEAD rewrite must protect just like a dirty rewrite. Add the exact public regression using a committed destination change.

## Prior-finding disposition

| Rereview-5 P2 | Disposition |
|---|---|
| Rewritten destination observation can be accepted | **Partially resolved.** Malformed collections and rewritten dirty observations now fail closed. Rewritten clean observations at a new HEAD remain accepted (P2-1). |

## Focused verification

- Intact context-only recovery regression: **PASS**.
- Rewritten dirty destination regression: **PASS**; 2 focused tests completed in 26.598 seconds.
- Reviewer public rewritten-dirty probe: **fail closed**, recovery ambiguous, original journal pending.
- Reviewer public rewritten-clean-new-HEAD probe: **confirmed P2-1**, recovery clean, original journal terminalized as failed.
...
src/skills/linear-delivery-loop/references/supervisor-core.md sha256:1276130e552b8892b3bedded456b9544af200216e0790aa20b19c36dfa559e04
...
src/skills/linear-delivery-loop/scripts/assembled_handoff.py sha256:bde5811aed3da07661d35ced1fbe4d7acfa33e2db8ef70d4b7cbd938e493eadc
...
tests/linear_delivery_supervisor/test_assembled_handoff.py sha256:4771d52c414a88a30cbc6a7288d645ccdf176f9116515899cbf08078df82d96c
...
Overall target digest: `sha256:b1c78c56a9882cf805767afebb090ab7f379a8c22a82c1b1dcf90345468ae604`.
